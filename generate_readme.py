...
def generate_readme():
    # 使用占位符避免 Markdown 渲染器提前截断代码块
    readme_content = """# 🤖 智能 AI Agent (多智能体协作版)
...
"""
    ...
    # 将占位符替换为真实的 Markdown 反引号
    readme_content = readme_content.replace("[BACKTICKS]", "```")

    file_path = "README.md"
    try:
        with open(file_path, "w", encoding="utf-8") as f:
            f.write(readme_content)
        print(f"✅ 成功生成 README 文件: {os.path.abspath(file_path)}")
    except Exception as e:
        print(f"❌ 生成 README 失败: {e}")
